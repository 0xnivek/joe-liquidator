...
describe("JoeLiquidator", function () {
  ...
  describe("Test liquidate ERC20 borrow position and ERC20 supply position", function () {
    ...
    it("Test liquidate USDT borrow position and LINK supply position", async function () {
      ...
      /// - AVAX is 18 decimals
      /// - is 18 decimals
      console.log("Swapping AVAX for LINK...");
      ...
      /// second for jLINK.
      const jUSDTBorrowRatePerSecond = await jUSDTContract.borrowRatePerSecond();
      const jLINKSupplyRatePerSecond = await jLINKContract.supplyRatePerSecond();
      expect(jUSDTBorrowRatePerSecond.gt(jLINKSupplyRatePerSecond)).to.equal(true);
